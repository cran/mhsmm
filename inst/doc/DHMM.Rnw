% \VignetteIndexEntry{HMM for discrete data}
...
\usepackage{a4wide}
\title{Using \texttt{mhsmm} for smoothing discrete data}
\author{Søren Højsgaard and Jared O'Connell}

\begin{document}


\SweaveOpts{prefix.string=fig/dhmm} %sæt dir til "fig" og prefix til "bar" for figurer
...
diag(B$pmf) <- .9
init.spec <- hmmspec(init,trans=P,emission=B,f=dpmf)
init.spec
@ %def 
